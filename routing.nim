...
func chainTree(rope: seq[BasePatternNode], handler: int = 0): PatternNode =
...
func merge(
  ...
  if rope.len == 1: # Terminating bnode reached, finish the merge
    result = terminatingPatternNode(node, rope[0], route)
  else:
    ...
    if childIndex == -1: # the next bnode doesn't map to a child of this node, needs to me directly translated into a deep tree (one branch per level)
      result.children.add(chainTree(remainder)) # make a node containing everything remaining and inject it
    else:
